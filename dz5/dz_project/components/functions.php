...
function getCategoryBySlug(string $slug): array
{
    ...
    if (empty($filtered)) {
        ...
        header('Location: /404.php?msg=' . urlencode('Категория с таким ID не найдена'));
        exit;
    }
...
function getPosts(): array
{
    try {
        ...
        error_log("Ошибка загрузки постов: " . $e->getMessage());
        // throw new RuntimeException("Не удалось загрузить посты");
        header('Location: /404.php?msg=' . urlencode('Посты не найдены'));
        exit;
    }
}
...
// function getCategoriess($posts)
// {
//     $postCategories = array_column($posts, 'category');
//     return array_unique($postCategories);
// }



// function filterByCategory($posts, $category): array
// {
